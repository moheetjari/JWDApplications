...
public class AddDepartment extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddDepartment</title>");
            out.println("</head>");
            out.println("<body>");
            ...
            try {
                ...
                if (DepartmentId == null) {
                    PreparedStatement stmt = con.prepareStatement("INSERT INTO DEPARTMENT(DEPARTMENTNAME) VALUES(?)");
                    stmt.setString(1, DepartmentName);
                    int rowsAff = stmt.executeUpdate();
                    if (rowsAff > 0) {
                        response.sendRedirect("GetDepartment");
                    }
                } else {
                    PreparedStatement stmt = con.prepareStatement("UPDATE DEPARTMENT SET DEPARTMENTNAME=? WHERE Id=?");
                    stmt.setString(1, DepartmentName);
